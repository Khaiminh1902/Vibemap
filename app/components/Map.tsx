...
export default function Map() {
  ...
  ${
    currentReaction === emoji
      ...
      <div
        className={`absolute ${
          viewMode === "map" ? "bottom-40" : "bottom-22"
        } right-4 z-[9999]`}
      >
        <button
          onClick={() => setViewMode(viewMode === "map" ? "globe" : "map")}
          className="bg-white hover:bg-white/80 cursor-pointer text-black px-4 py-2 rounded-md shadow-md"
        >
          {viewMode === "map" ? "Switch to Globe View" : "Switch to Map View"}
        </button>
      </div>
